...
func main() {
	...
	// orchestrator for a given target. It is called from the GUI picker callback.
	buildScan := func(pickedTarget, pickedLocalPath, pickedOutputDir, pickedRetestPath string) (sessionResult, error) {
		return buildScanSession(scanBuildConfig{
			defaultRetestPath: retestPath,
			pickedTarget:      pickedTarget,
			pickedLocalPath:   pickedLocalPath,
			pickedOutputDir:   pickedOutputDir,
			pickedRetestPath:  pickedRetestPath,
			...
			sastCfgDir:        sastCfgDir,
			gemmaThinking:     *gemmaThinkingReq,
			reservedHostPorts: reservedHostPorts,
