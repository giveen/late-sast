...
func (t DocsResolveTool) CallString(args json.RawMessage) string {
	q := tool.GetToolParam(args, "query")
	if q == "" {
		q = "unknown"
	}
...
func (t DocsReadTool) CallString(args json.RawMessage) string {
	u := tool.GetToolParam(args, "url")
	if u == "" {
		u = "unknown"
	}
	return fmt.Sprintf("docs_read(%s)", tool.Truncate(u, 60))
}
...
func (t DocsSearchTool) CallString(args json.RawMessage) string {
	u := tool.GetToolParam(args, "url")
	q := tool.GetToolParam(args, "query")
	return fmt.Sprintf("docs_search(%s, %q)", tool.Truncate(u, 40), q)
}
