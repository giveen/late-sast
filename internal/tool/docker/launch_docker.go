...
func ensureDockerNetwork(ctx context.Context, runner tool.CommandRunner, networkName string) error {
...
func launchCompose(
	...
	if patchNetwork {
		...
		patched, _, err := tool.PatchComposeNetwork(raw, networkName)
		if err != nil {
...
func inspectPortInfo(ctx context.Context, runner tool.CommandRunner, target string) map[string]any {
	...
	if err := json.Unmarshal([]byte(raw), &ports); err != nil {
		return map[string]any{"raw": tool.Truncate(raw, 500)}
	}
...
func inspectNameAndPorts(ctx context.Context, runner tool.CommandRunner, target string) (string, map[string]any) {
	const sep = "||LATE_SEP||"
...
// docker inspect round-trip using a delimited format template.
func inspectIDAndPorts(ctx context.Context, runner tool.CommandRunner, target string) (string, map[string]any) {
	const sep = "||LATE_SEP||"
	out, err := runner(ctx, "docker", "inspect", "-f",
