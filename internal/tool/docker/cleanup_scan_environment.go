...
func (t CleanupScanEnvironmentTool) Execute(ctx context.Context, args json.RawMessage) (string, error) {
	...
	appendStep := func(name, command string, out string, err error) {
		step := map[string]any{
			...
			"command": command,
			"ok":      err == nil,
			"output":  tool.Truncate(strings.TrimSpace(out), 500),
		}
		if err != nil {
			step["error"] = tool.Truncate(err.Error(), 200)
		}
	...
	if len(composeFiles) > 0 {
		...
		for _, cf := range composeFiles {
			...
			msg := fmt.Sprintf("%s: %s", cf, tool.Truncate(strings.TrimSpace(cfOut), 150))
			if cfErr != nil {
				msg += " (err: " + tool.Truncate(cfErr.Error(), 120) + ")"
			}
	...
	collectIDs := func(filterType, filterValue string) {
		...
		if listErr != nil {
			filterNotes = append(filterNotes, fmt.Sprintf("%s=%s err: %s", filterType, filterValue, tool.Truncate(listErr.Error(), 120)))
			return
		}
	...
	if absErr != nil {
		...
		cleanupWorkdirCmd := fmt.Sprintf("rm -rf %s", tool.ShQuote(absWorkdir))
