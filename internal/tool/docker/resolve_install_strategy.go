...
func (t ResolveInstallStrategyTool) CallString(args json.RawMessage) string {
	githubURL := tool.GetToolParam(args, "github_url")
	if githubURL == "" {
...
func (t ResolveInstallStrategyTool) Execute(ctx context.Context, args json.RawMessage) (string, error) {
	...
	if readmeErr != nil {
		notes = append(notes, "README fetch failed: "+tool.Truncate(readmeErr.Error(), 200))
	}
	if releaseErr != nil {
		notes = append(notes, "release metadata fetch failed: "+tool.Truncate(releaseErr.Error(), 200))
	}
...
func (t ResolveInstallStrategyTool) httpGET(ctx context.Context, reqURL string) (string, error) {
	...
	if resp.StatusCode < 200 || resp.StatusCode >= 300 {
		return "", fmt.Errorf("GET %s: status %d: %s", reqURL, resp.StatusCode, tool.Truncate(string(data), 200))
	}
