...
func (t RunSecretsScannerTool) Execute(ctx context.Context, args json.RawMessage) (string, error) {
	...
	cmd := fmt.Sprintf("trufflehog filesystem --json%s --no-update --fail --results=verified,unknown %s 2>/dev/null", verifiedFlag, tool.ShQuote(p.ScanPath))
...
func (t RunSecretsScannerTool) ensureTruffleHog(ctx context.Context, runner tool.CommandRunner, container string) (bool, error) {
...
func parseTruffleHogJSONL(raw string, onlyVerified bool, maxFindings int) ([]TruffleHogFinding, error) {
	...
	for s.Scan() {
		...
		findings = append(findings, TruffleHogFinding{
			...
			File:     file,
			Line:     lineNo,
			Redacted: tool.Truncate(redacted, 120),
			Source:   source,
			Category: category,
		})
