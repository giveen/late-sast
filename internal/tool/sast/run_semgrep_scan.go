...
func (t RunSemgrepScanTool) Execute(ctx context.Context, args json.RawMessage) (string, error) {
	...
	if parseErr != nil {
		result := map[string]any{
			...
			"raw_output": tool.Truncate(strings.TrimSpace(raw), 3000),
			"findings":   []any{},
			"total":      0,
		}
		out, _ := json.Marshal(result)
...
func (t RunSemgrepScanTool) ensureOpenGrep(ctx context.Context, runner tool.CommandRunner, container string) (bool, error) {
...
func (t RunSemgrepScanTool) detectLanguage(ctx context.Context, runner tool.CommandRunner, container, scanPath string) string {
...
func parseOpenGrepJSON(raw string, severityFilter []string, maxFindings int) ([]semgrepFinding, error) {
	...
	for _, r := range out.Results {
		...
		findings = append(findings, semgrepFinding{
			...
			Column:   r.Start.Col,
			Severity: sev,
			Message:  tool.Truncate(strings.TrimSpace(r.Extra.Message), 300),
			CWE:      cwe,
			OWASP:    owasp,
