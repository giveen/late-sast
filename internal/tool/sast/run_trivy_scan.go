...
func (t RunTrivyScanTool) Execute(ctx context.Context, args json.RawMessage) (string, error) {
	...
	if err != nil {
		...
		result := map[string]any{
			...
			"raw_output": tool.Truncate(strings.TrimSpace(tableOut), 4000),
			"findings":   []any{},
			"total":      0,
		}
...
func (t RunTrivyScanTool) ensureTrivy(ctx context.Context, runner tool.CommandRunner, container string) (bool, error) {
...
func parseTrivyJSON(raw string, cvssThreshold float64) ([]TrivyFinding, string, int, error) {
	...
	for _, result := range report.Results {
		for _, v := range result.Vulnerabilities {
			...
			if desc == "" {
				desc = tool.Truncate(strings.TrimSpace(v.Description), 200)
			}
