...
### Fixed
- **Setup Step 2: phantom `index_repository`/`get_architecture` calls** (`instruction-sast-setup.md`): these MCP tool names are not present in the setup agent's tool list. The agent fell back to calling `ctx_index_file` with the entire repo directory, producing `"is a directory"` errors and wasting a turn before adapting. Step 2 now uses `bash find` to discover source files and instructs the agent to call `ctx_index_file` on individual files only, with an explicit warning against passing directory paths.
